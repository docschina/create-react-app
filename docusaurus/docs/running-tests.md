...
虽然 Jest 通过 [jsdom](https://github.com/tmpvar/jsdom) 提供了像 `window` 这样的全局变量，但它们仅仅类似于真正浏览器的行为。Jest 旨在用于逻辑和组件的单元测试，而不是 DOM 行为。

如果你需要它们，我们推荐你使用单独的工具进行浏览器端到端测试。它们超越了 Create React App 的范围。

## 文件命名规范

Jest 将查找符合以下流行命名规则的测试文件：
...
我们建议把测试文件（或 `__tests__` 文件夹）放到正测试的代码旁边，以使相对导入显得更短。例如，如果 `App.test.js` 和 `App.js` 在同一文件夹下，测试文件仅仅需要 `import App from './App'`，而不需要较长的相对路径。这样的搭配也有助于在更大的项目中更快地找到测试文件。
...
所有的 Jest 匹配器[在这里详细记录](https://jestjs.io/docs/en/expect.html)。
...
作为 `enzyme` 的替代品或同伴，你可以考虑采用 `react-testing-library`。[`react-testing-library`](https://github.com/testing-library/react-testing-library) 是用于测试 React 组件的库，其方式类似于最终用户使用组件的方式。它非常适合对 React 组件和应用进行单元测试、集成测试和端到端测试。它与 DOM 节点一起工作更加直接，因此建议与 [`jest-dom`](https://github.com/testing-library/jest-dom) 一起使用，以改进断言。 
...
这是一个使用 `react-testing-library` 和 `jest-dom` 的例子，它用于测试 `<App />` 组件渲染“Welcome to React”。
